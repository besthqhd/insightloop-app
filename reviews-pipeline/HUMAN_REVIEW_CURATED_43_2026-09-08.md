...
| 131 | DeepSeek | 白天代码一遍过，深夜需反复修改 | positive | negative | mixed* | 当前产品有明确分时段优点和缺点；是否以主诉为 negative 需确认。 |
...
- GLM 与候选标签完全一致：**36/43**。
- 按现有 Eval 的负面大类口径（`negative ≈ angry`），GLM 与候选标签一致：**38/43**。
- GLM 的 5 个负面大类未命中为 #112、#128、#131、#134、#141；其中四条被判为 `neutral`，一条分时段褒贬被判为 `negative` 而候选为 `mixed`。
- 上述数字只用于定位数据与模型问题。由于标签由可见模型输出的单人初审产生，**不能对外称为人工准确率**。

## 待负责人确认的 7 条

1. #104：`negative`——措辞尖锐但未达到辱骂、威胁或弃用。
2. #109：`negative`——“信任崩塌”强烈，但仍以具体诚信问题为主。
3. #113：`angry`——“笨”视为直接辱骂；若你的 angry 阈值更高，可改为 negative。
4. #130：`angry`——“没读过书”视为直接侮辱；若阈值更高，可改为 negative。
5. #131：`mixed`——明确说白天表现好、深夜表现差，两种当前体验同时存在。
6. #134：`negative`——当前版本是主诉，过去表现好不算当前正面。
7. #141：`negative`——反问是讽刺性差评，不是无态度的事实陈述。

确认后再把 32 条修订写入版本化标签文件，并在同一固定配置上重跑；本轮不改 Prompt，以便把标签校准效应与模型改动分开。
