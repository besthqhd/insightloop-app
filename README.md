...
- 仓库内的 **143 条**数据是 ChatGPT、Kimi、豆包、腾讯元宝和 DeepSeek 的公开竞品评论，服务于反馈分析与 Evals 演示，**不是 InsightLoop 的项目用户或客户数据**。
- 洞察看板可切换到 **真实用户测试 v1**：8 位参与者提出的 23 条合并意见。界面同时展示记录数与去重参与者数、主题证据 ID、模型建议和人工决策；本轮 3 项机会均由项目负责人标记为「采纳」，但这不等于方案效果已经验证。
- 只有人工标记为「采纳」的机会才能继续生成 PRD、验收标准和埋点草稿；暂缓或驳回会禁用生成入口，决策理由保存在浏览器本地。
- 审核后的最终方案可一键「转入机会工作区」，自动携带建议优先级、证据 ID、人工采纳理由和 PRD 内容；同一机会再次转入会更新原卡片，避免重复。
- 页面中的「准确性 / 幻觉风险」主题为其中 **9 / 143** 条明确相关反馈形成的待验证信号；它不能推断发生率、市场规模、用户量、商业效果或最终优先级。
- 没有来源的经营、行为或上线效果数字不作为项目事实。AI 输出若没有输入证据，只能标为「待验证假设」；最终采纳由人工完成。
- 数据来源、清洗、脱敏和选择偏差见 [`reviews-pipeline/DATA_README.md`](./reviews-pipeline/DATA_README.md)。
...
- **评估看板（Evals）· 量化 AI 质量（v2.0–v2.2.8）**：用 **143 条公开竞品评论**跑 `callModel` 链路（Mock 或真实模型），实时计算三大指标：① 格式稳定率 ② 至少一项字段命中率 ③ 可行动代理通过率。后两项不是完整语义理解率或人工满意度。2026-09-08 在人工标签修订 v2（累计 63 条）上的单次实跑结果为：真实模型（GLM-4-Flash）100% / 83% / 94%；Mock 关键词规则基线 100% / 29% / 23%。该结果来自逐例导出的本地审计 JSON，仍是单次运行，不应外推为总体模型能力。实测界面见 [`evals-evidence.svg`](./evals-evidence.svg)。
...
**评估看板「对比 Mock vs 真实」模式（v2.1–v2.2.7）：** 评估看板可切换「当前模型 / 对比 Mock vs 真实」。选「对比」并配置好后端（或直连 Key）后，点「运行评估」会用**同一套 143 条真实竞品评论**分别跑 Mock 关键词规则基线与真实模型（GLM-4-Flash），并以可控并发（并发 2）渲染逐用例对比表（字段命中/可行动代理 ✓✗、是否有差异）。× 表示未达到对应评测规则，不表示调用失败。实测结果见 [`evals-evidence.svg`](./evals-evidence.svg) 与 PRD 第 10 节。

### 可复现运行产物

`reviews-pipeline/run_real_eval.cjs` 默认跑 Mock，只有设置 `INSIGHTLOOP_USE_REAL=1` 才会调用真实模型。每次运行会在 `reviews-pipeline/runs/` 留下逐例结果、无密钥元数据和截图；详细命令、产物字段与安全边界见 [`reviews-pipeline/runs/README.md`](./reviews-pipeline/runs/README.md)。

真实模型的数字只有在对应逐例运行产物存在时才可作为审计证据；不要以旧截图或汇总数字替代。
