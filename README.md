...
- **评估看板（Evals）· 量化 AI 质量（v2.0–v2.2.8）**：新增「评估看板」页，用 **143 条真实竞品评论**（ChatGPT / Kimi / 豆包 / 元宝 / DeepSeek）跑 `callModel` 链路（mock 或真实模型），实时计算三大指标：① 格式稳定率 ② 任务理解率 ③ 有帮助率。支持「对比 Mock vs 真实」并排（可控并发 2 + 进度提示），并有「测试连接」诊断与 25s 超时 + 重试兜底。核心立意——**LLM 输出是概率性的，必须被度量，而非只会「能跑」**。在 143 条真实评论上实测：真实模型（GLM-4-Flash）格式稳定率 100% / 任务理解率 53% / 有帮助率 99%；Mock 基线任务理解率 66% / 有帮助率 21%。实测结果见 [`evals-evidence.svg`](./evals-evidence.svg)。
