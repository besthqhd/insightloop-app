...
7) 数据测试与 Evals：10–20 组测试数据 + 三问判定(理解任务/格式稳定/有帮助) + 测试报告 harness；进阶为 Mock vs 真实 100 用例量化看板
...
进阶为 Evals 看板：100 条边界用例（正常/缺失/仅情绪/过长/攻击/多意图+风险/正向/英文/单意图/时间等），支持「对比 Mock vs 真实」：同套用例分别跑 mock 与真实模型，逐条 ✓✗ 标差异；未配置 Key 只跑 mock 并提示。
```
...
> 从 0 搭建 AI 产品：先用结构化澄清把模糊想法落地为 PRD + 竞品分析对齐范围，再以单 HTML 文件直出高保真可交互原型（设计即出码，跳过 Figma 翻译）；设计 AI 契约层（失败状态机 + repairJson 兜底）与三层展示法（状态/追溯/日志）；以"7 基础 + 6 亮点"UX 审计闭环，多轮死按钮消除保障交互可靠；建立 10–20 组测试 + 100 条边界用例的 Mock 基线 vs 真实模型（GLM-4-Flash，OpenAI 兼容 + 密钥托管后端）量化对比，格式稳定率 100%、Mock 理解/帮助 88%/90%、真实模型理解/帮助 97%/97%；据评估迭代超时重试 + 并发限速 + 置信校准；全站 localStorage 持久化。
