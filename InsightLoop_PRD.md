...
| FR-10.5  | 实测基准（v2.2.7，GLM-4-Flash + 密钥托管后端，15 条边界用例）：格式稳定率 100%（零超时）；Mock 与真实模型在 **13/15 用例结果完全一致**，差异集中在「信息缺失」「纯情绪无事实」两类边界文本的把握度策略；任务理解率约 93%、有帮助率约 87%（两者高度吻合）。用于模型路由与成本/质量权衡叙事 |
...
| 任务理解率 | ~93% | ~93% | 至少命中一个预期维度 |
| 有帮助率 | ~87% | ~87% | 格式稳定且把握度非 low / 命中客诉升级 |

- **13/15 用例两模型结果完全一致**；剩余 2 条差异均为"真实模型把握度策略更保守 / 更谨慎"的表现（如「仅情绪无事实」真实模型不给高把握，反而更合理），并非模型错误。
...
`evals-evidence.svg` 由评估看板实测数据生成（格式 100% / 理解 93% / 帮助 87%，Mock 与 GLM-4-Flash 高度吻合）。若需嵌入**你自己的精确截图**，将截图保存为同路径 `evals-evidence.png`（或 `.svg`）覆盖即可，文档引用无需改动。
