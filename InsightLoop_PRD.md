...
版本：v1.5 ｜ 日期：2026-07-28

> 本文档定义产品的需求与设计约束，作为 UI 设计与开发落地的依据。  
> 产品定位：一个能够从用户反馈中发现高价值问题，并推动产品完成迭代验证的 AI 产品经理助手。  
> **v1.5 更新说明**：修复 `ai-contract.js` 缺失 `callRealModel` 导出导致 AI 模块加载失败的隐藏 bug；新增草稿机会编辑弹窗与导入反馈 CSV/JSON 解析；JS module 增加 cache-buster 避免 CDN 缓存旧版。
...
| FR-7.9 | 本地持久化（刷新不丢）：人工「采纳 / 点赞点踩」、双击编辑内容、活动日志与版本历史落盘到浏览器 localStorage（独立于 AI 密钥的两个 key），刷新或重开页面后原样还原；「⚙ AI 设置」提供「清空本地记录」一键重置，不影响 AI 密钥（v1.2 已实现 UI） |
| FR-7.10 | 反馈中心实时搜索 / 筛选：搜索框与来源 / 用户类型 / 情绪 / 主题 / 时间筛选联动，输入或勾选即过滤列表并更新选中计数；支持按时间排序与一键重置（v1.3 已实现 UI） |
| FR-7.11 | 判断依据透明面板：洞察看板「AI 结论」与机会工作区「AI 优先级评分」可展开「判断依据」——证据链 → 归因 → 优先级判定 / 四维评分分解的推理链，结论可解释、可溯源（v1.3 已实现 UI） |
| FR-7.12 | 失败演练与三层容错兜底：活动日志面板内置「失败模式」开关（正常 / 超时 / 脏 JSON）与「一键演示容错」按钮，触发后现场展示「状态徽标 → 多智能体追溯抽屉 → 活动日志」三层容错；真实模型模式下演练同样生效（v1.3 已实现 UI） |
| FR-7.13 | 全站按钮交互可靠性修复：所有页面按钮/下拉统一使用稳定 `id` 或事件委托绑定，消除 `byText` 文本匹配在下拉切换后的失效风险；洞察/机会卡片点击改为事件委托以支持动态新建卡片；AI 复盘/建议渲染增加防御性处理，避免字段缺失时显示 `undefined`（v1.4 已实现 UI） |
| FR-7.14 | 草稿机会编辑：点击「新建机会」或草稿卡片弹出编辑弹窗，可填写/修改标题、来源、优先级、描述、用户故事、PRD；保存后同步刷新右侧详情面板（v1.5 已实现 UI） |
| FR-7.15 | 导入反馈 CSV / JSON 解析：导入按钮按导出格式解析 CSV（兼容中英文表头）或 JSON 数组并追加到反馈列表；提供导入模板下载，格式错误时弹窗说明（v1.5 已实现 UI） |
