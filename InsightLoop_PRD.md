...
| FR-10.5  | 实测基准（v2.2.8，100 条固定边界反馈用例，经 Node 校验器逐条对拍）：确定性 Mock 基线达到格式稳定率 100%、任务理解率 88%、有帮助率 90%（零超时）；剩余失分项均为单意图来源定位、正向 / 英文情绪识别等 Mock 中文基线的已知盲区。真实模型（GLM-4-Flash + 密钥托管后端）对比需重跑 Evals 看板确认，用于模型路由与成本 / 质量权衡叙事 |
...
- **v2.2 新增**：可在「⚙ AI 设置」配置真实模型并"测试连接"通过；评估看板可跑通 Mock vs 真实模型对比，真实模型（GLM-4-Flash）格式稳定率达 100%（实测 15 条边界用例）。
...
| --- | --- | --- | --- |
| 格式稳定率 | 100%（已验证） | 待重跑 Evals 看板确认 | 输出可解析 + schema 校验通过，零超时 |
| 任务理解率 | 88%（已验证） | 待重跑 Evals 看板确认 | 至少命中一个预期维度 |
| 有帮助率 | 90%（已验证） | 待重跑 Evals 看板确认 | 格式稳定且把握度非 low / 命中客诉升级 |

- **100 条用例的确定性 Mock 基线已逐条验证**（格式 100% / 理解 88% / 帮助 90%）；剩余失分项集中在单意图来源定位、正向 / 英文情绪识别等 Mock 中文基线的已知盲区，接入真实模型后显著收敛。真实模型对比结论需重跑 Evals 看板后更新，不在此预后。
...
`evals-evidence.svg` 由评估看板实测数据生成（v2.2.8，100 条用例，Mock 确定性基线：格式 100% / 理解 88% / 帮助 90%）。若需嵌入**你自己的精确截图**（含真实模型对比），将截图保存为同路径 `evals-evidence.png`（或 `.svg`）覆盖即可，文档引用无需改动。
