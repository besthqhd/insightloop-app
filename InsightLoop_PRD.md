...
版本：v3.0（结构重构）｜ 日期：2026-07-30

> 本文档是 InsightLoop 产品的**单一事实源（single source of truth）**，定义"要建什么"与"为什么建"，作为设计、研发、测试、评审各部门的共同依据。  
...
- **产品**：InsightLoop（AI 产品迭代闭环工作台）
- **版本**：v3.0 ｜ **日期**：2026-07-30
- **读者与用途**（本文档面向全员，按读者跳读对应章节）：
  - **产品 / 评审**：背景、目标、范围、指标、路线图 → 正文 1–4、8、11
  - **设计**：交互 / UX 规格、AI 协同状态规范、设计系统令牌 → 正文 6 + 附录 C
  - **研发**：功能需求、非功能需求、技术架构参考 → 正文 6–7 + 附录 A
  - **测试**：验收标准、Evals 实测证据 → 正文 10 + 附录 B
- **实现状态快照**见附录 D；本正文不逐行标注实现状态，避免规格与进度混淆。
- **文档性质**：当前产品以"前端可交互 Demo + 密钥托管后端"形态交付（作品集 / 演示级），正文描述的是**产品约定**，原型 / 生产的边界见正文 14 与附录 D。
...
### 4.2 Out of Scope / Non-goals（不做什么）

> 明确边界，防止跨部门阅读时"顺手加需求"导致范围蔓延。

- **不做通用客服 / 工单系统**：InsightLoop 聚焦"反馈 → 洞察 → 机会 → 验证"闭环，不替代客服台或工单流转。
- **不做多租户 / 账号体系（MVP 阶段）**：采用本地 + 演示态，不做企业级权限 / SSO / 组织管理。
- **不内置真实语义向量数据库（MVP）**：先用本地 hash 向量 RAG，后续再升级为 embeddings。
- **不承诺真实用户 DAU / 商业收入指标**：当前为作品集 / 演示产品，量化以 Evals 三率与流程成功率为准，不虚构用户规模。
- **不做自动执行排期 / 下发**：AI 只生成草稿与建议，决策与执行由人完成（见 NFR-3 可控）。
- **不支持浏览器直连第三方 LLM Key 上线**：生产态必须经密钥托管后端，避免 Key 下发到客户端。
...
- 作为 PM，我希望在评估看板对比 Mock 与真实模型，以便用数据做模型选型决策。
...
- **前端**：单文件 `index.html`（1440px 等比缩放，纯 CSS / 原生 JS，零外部资源），离线可打开、易托管。
- **逻辑层**：`ai-contract.js`（ES module，纯逻辑）——固定 JSON 契约、8 个 SCHEMAS、失败状态机、版本化重生、真实模型调用。
...
- **AI 模型**：双轨——`mockModel`（确定性基线）与真实 `callRealModel`（GLM-4-Flash 等），通过「⚙ AI 设置」切换。

### A.2 部署

- 托管：GitHub Pages，来源 `main` 分支根目录，公开 + 强制 HTTPS。
- 线上原型：<https://besthqhd.github.io/insightloop-app/>
- 本地运行：`python -m http.server 8080` 后访问（因 ES module，建议起本地服务而非 `file://`）。
- 密钥托管后端：将 `proxy-worker.js` 部署到 Cloudflare Worker，添加 Secret `ZHIPU_API_KEY`；前端「后端地址」填 `https://<your-worker>.<subdomain>.workers.dev/v1`。

### A.3 接入真实模型的改动点

- 浏览器侧不传 Key：填写 API Base URL / 模型名 / **后端地址**（密钥托管后端），勾选"使用真实模型"即可。
- 请求链路：浏览器 → 密钥托管后端（注入 Secret Key）→ 智谱等 OpenAI 兼容 API；响应原样回传。
- 无需改动契约与 UI 层：`callRealModel` 复用 `buildContext` / `buildSystemPrompt` 输出，回传 JSON 供 `repairJson` + `validate` 处理。
- 可用性兜底：25s 超时 + 失败重试 1 次 + "测试连接"诊断。
...
- 固定 `EVAL_CASES`（**100 条**），`evalOne` 执行单条并判定三指标。
- `runEvalSuite`：`current` 跑当前模型，`compare` 跑 Mock + 真实并排。
- 可控并发执行器（并发 2）+ 进度提示，结果由渲染函数呈现。

---

## 附录 B · Evals 实测证据（测试 / 高管用）

> 以下为「评估看板 → 对比 Mock vs 真实」在 **100 条**固定边界反馈用例上的实测结果（可复现：线上 demo 顶栏「⚙ AI 设置」配置密钥托管后端后运行即得一模一样数据，参考日期 2026-07-28）。
...
| ----- | --------- | --------------- | ----------------------- |
| 格式稳定率 | 100%      | 100%           | 输出可解析 + schema 校验通过，零超时 |
| 任务理解率 | 88%       | 97%            | 至少命中一个预期维度              |
| 有帮助率  | 90%       | 97%            | 格式稳定且把握度非 low / 命中客诉升级  |

- 100 条用例双轨对比已实测：确定性 Mock 基线格式 100% / 理解 88% / 帮助 90%；真实模型 GLM-4-Flash 格式 100% / 理解 97% / 帮助 97%。两者高度吻合且零超时，真实模型在正向表扬、英文反馈等边界 case 上明显优于 Mock 中文关键词基线；表扬 + 建议混合、重复刷屏、部分中性建议等细语义场景仍有少量 miss，已逐条在评估看板中标注。
- 核心结论：**LLM 输出必须被量化**；mock 不是"更准"，而是"确定性规则"；真实模型的微小差异暴露边界场景的置信策略问题，指导 prompt 校准与判定标准优化。
...
> 本节集中记录"已实现 / 已上线"状态，便于评审快速判断进度；正文规格不受此影响。
...
| 三层展示法      | 内联状态 / 抽屉 / 活动日志          | ✅ 已原型      | 见附录 C，规范帧在 Ardot 画布                                       |
| 真实 LLM 接入  | 后端 / API / 密钥托管 / 评测      | ✅ 已实现      | GLM-4-Flash + 密钥托管后端（Cloudflare Worker Secret）               |
| 本地 RAG 检索  | hash 向量 / 余弦 / Top-k      | ✅ 已实现      | 零依赖本地证据检索，余弦相似度 + 阈值 0.12                             |
| Evals 量化看板 | Mock vs 真实对比 / 并发评估       | ✅ 已实现      | 格式稳定率 / 任务理解率 / 有帮助率；**100 条边界用例已上线（v2.2.8）**                |

> 原型为**前端可交互 Demo**：AI 结果可由 `mockModel`（确定性基线，用于回归与可复现）或真实模型（GLM-4-Flash，经密钥托管后端）生成，通过「⚙ AI 设置」切换；模型为 mock 时不消耗真实 token。线上 demo 截至 2026-07-30 已部署 **v2.2.8**（100 条 Evals 用例生效）。
